 ...
 * 
 * Utilizzo:
 * node scripts/configure-deployment.cjs --mode=[root|subdirectory]
 */
...
if (!mode || (mode !== 'root' && mode !== 'subdirectory')) {
  ...
  console.error('node scripts/configure-deployment.cjs --mode=[root|subdirectory]');
  process.exit(1);
}
...
// 3. Aggiorna il file vite.config.ts
...
console.log(`✅ Aggiornato vite.config.ts con base = '${basePath}'`);
