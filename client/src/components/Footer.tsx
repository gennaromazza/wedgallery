...
export default function Footer() {
  ...
  return (
    <footer className="bg-blue-gray">
      <div className="max-w-7xl mx-auto py-12 px-4 overflow-hidden sm:px-6 lg:px-8">
        {/* Logo e Info dello Studio */}
        <div className="flex flex-col md:flex-row items-center justify-between mb-8">
          <div className="flex flex-col items-center md:items-start mb-6 md:mb-0">
            {studioSettings.logo ? (
              ...
              <h3 className="text-2xl font-playfair text-off-white mb-3">{studioSettings.name}</h3>
            )}
            {studioSettings.slogan && (
              <p className="text-sm text-cream italic">{studioSettings.slogan}</p>
            )}
          </div>
          
          <div className="text-center md:text-right">
            {studioSettings.address && (
              <p className="text-off-white mb-1">{studioSettings.address}</p>
            )}
            {studioSettings.phone && (
              <p className="text-off-white mb-1">
                <a href={`tel:${studioSettings.phone}`} className="hover:text-cream">
                  {studioSettings.phone}
                </a>
              </p>
            )}
            {studioSettings.email && (
              <p className="text-off-white">
                <a href={`mailto:${studioSettings.email}`} className="hover:text-cream">
                  {studioSettings.email}
                </a>
              </p>
            )}
        ...
        <div className="mt-8 flex justify-center space-x-6">
          {studioSettings.socialLinks?.facebook && (
            <a 
              ...
              className="text-off-white hover:text-cream"
            >
        ...
        {studioSettings.socialLinks?.instagram && (
          <div className="mt-6 text-center">
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer"
              className="inline-flex items-center px-4 py-2 bg-sage text-blue-gray rounded-md hover:bg-sage/90 transition"
            >
